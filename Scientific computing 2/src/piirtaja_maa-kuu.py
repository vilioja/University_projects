...
aika = float(sys.argv[2])			#luetaan komentoriviltä mikä oli laskettaessa talletusaikojen väli päivissä
x = 0
for line in f:
	plt.close('all')
	fig, (ax1, ax2) = plt.subplots(1, 2)
	ax1.axis([-0.01,0.01,-0.01,0.01])
	ax2.axis([-0.01,0.01,-0.01,0.01])
	ax1.set_aspect(1)
	ax2.set_aspect(1)
	line.rstrip('\n')
	rivi = line.split(';')	
	...
	fig.text(0.5, 0.2, 'Distance (AU), T = %8.4f d' % (x*aika), ha='center', va='center')
	fig.text(0.05, 0.5, 'Distance (AU)', ha='center', va='center', rotation='vertical')
