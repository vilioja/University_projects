...
f = sys.argv[1]
filut = np.genfromtxt(f,dtype='string')
maara = filut.shape[0]
sima = np.zeros(maara)
sime = np.zeros(maara)
simq = np.zeros(maara)
...
for i in range(0,maara):
    ...
    m1 = data[0, pointoffset]
    m2 = data[0, pointoffset + pointcomps]
    starta = data[0, pointoffset + pointcomps + oeoffset]
    starte = data[0, pointoffset + pointcomps + oeoffset + 1]
    endt = data[-1, 0]
    enda = data[-1, pointoffset + pointcomps + oeoffset]
    sima[i] = starta
    sime[i] = starte
    simt[i] = endt
    simq[i] = m2/m1
    ...
    integ = quad(lambda x: (x**(29./19) * (1 + (121./304)*(x**2))**(1181./2299))/(1 - x**2)**(3./2), 0, e)
    c0 = a * (1 - e**2) / ((e**(12./19)) * (1 + (121./304) * e**2)**(870./2299))
    b = (64./5) * (g**3 * redmass * totmass**2) / c**5
    t = (12./19) * (c0**4 / b) * integ[0]
    # while(a > alimit):
    #     dt = 2*np.pi*np.sqrt((a**3)/(g*totmass))
    #     da = -(64./5)*((g**3 * redmass * totmass**2)/(c**5 * a**3))*((1 + (73./24) * e**2 + (37./96) * e**4)/((1 - e**2)**(7./2)))
    #     de = -(304./15)*e*((g**3 * redmass * totmass**2)/(c**5 * a**4))*((1 + (121./304) * e**2)/((1 - e**2)**(5./2)))
    #     a = a + da
    #     e = e + de
    #     t = t + dt
    #     #print a * (1./au2m)
...
fig = plt.figure()
if 0:
    plt.plot(sima, simt, linestyle='-', color='blue')
    plt.plot(sima, anat, linestyle='--', color='orange')
    #plt.scatter(sima, simt)
    #plt.scatter(sima, anat)
    plt.xlabel(r'\textbf{Initial semimajor axis, AU}')
    plt.ylabel(r'\textbf{Time of merging, yr}')
    plt.tight_layout()
    plt.show()
if 0:
    plt.plot(sime, simt, linestyle='-', color='blue')
    plt.plot(sime, anat, linestyle='--', color='orange')
    plt.xlabel(r'\textbf{Initial eccentricity}')
    plt.ylabel(r'\textbf{Time of merging, yr}')
    plt.tight_layout()
    plt.show()
if 1:
    plt.plot(simq, simt, linestyle='-', color='blue')
    plt.plot(simq, anat, linestyle='--', color='orange')
    plt.xlabel(r'\textbf{Mass ratio of the BHs}')
    plt.ylabel(r'\textbf{Time of merging, yr}')
    plt.tight_layout()
    plt.show()
